...
sudo adduser --ingroup hadoop hduser # 新建用户
```
**注意：**出现这个报错'"hduser is not in the sudoers file. This incident will be reported."',可以通过以root用户身份登录解决此错误
...
* 3.下载Hadoop
	...
	* 3.3 选择tar.gz文件（文件名没有'src'）
    ![](/image/大数据03/大数据03_003.png)

* 4.放到服务器解压
```bash
sudo tar xzf hadoop-3.2.1.tar.gz # 解压
sudo mv hadoop-3.2.1 hadoop # 修改文件名
sudo chown -R hduser:hadoop hadoop # 目录增加用户权限
```
...
配置前：
![](/image/大数据03/大数据03_004.png)

配置后：	   
![](/image/大数据03/大数据03_005.png)
...
```bash
sudo chown -R hduser:Hadoop <在上一步中创建的目录路径>
...
```
![](/image/大数据03/大数据03_008.png)
* 3.创建以上设置中指定的目录并授权
```bash
sudo mkdir -p /home/hduser/hdfs
sudo chown -R hduser:hadoop /home/hduser/hdfs
sudo chmod 750 /home/hduser/hdfs
```
